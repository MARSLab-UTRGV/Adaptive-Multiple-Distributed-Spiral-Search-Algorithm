...
void DSA_loop_functions::generateSpiralPath()
{
    ...
    for (int i_region = 0; i_region < NumOfRegions; i_region++)
    {
        ...
        //reversedSpiralPoints.push_back(pointVector);
        pointVector.clear();   
...
void DSA_loop_functions::PostExperiment() 
{
    ...
    ofstream DataOut((FilenameHeader+"MDBSA-C-Data.txt").c_str(), ios::app);
    if (DataOut.tellp()==0){
    ...
    ofstream DataOut2((FilenameHeader+"MDBSA-C-TargetsPerMin.txt").c_str(), ios::app);
    if (DataOut2.tellp()==0){
